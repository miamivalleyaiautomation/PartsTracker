...
function renderPartCard(partNumber, part, expanded, onlyLoc) {
  ...
  card.className = 'card';
  card.id = 'part-' + partNumber.replace(/[^a-zA-Z0-9_-]/g,'_');
...
function initEvents() {
  ...
  if (els.partSearch) {
    ...
    els.partSearch.addEventListener('keydown',(e)=>{ if(e.key==='Enter'){ if(handleScanEnter()){ e.preventDefault(); } } });
...
if (typeof document!=='undefined'){document.addEventListener('click',(e)=>{const t=e.target.closest&&e.target.closest('#themeToggle');if(t){e.preventDefault();toggleTheme();}const s=e.target.closest&&e.target.closest('#scanToggle');if(s){e.preventDefault();toggleScan();}});}
